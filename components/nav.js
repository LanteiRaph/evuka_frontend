    ...
    return (
        <nav className='flex items-center bg-white justify-between shadow-2xl text-gray-900 py-4  md:py-2 px-6'>
            ...
            <div className="w-5/12 md:w-4/12 lg:w-2/12 flex justify-between items-center ">
                ...
                {!user ?
                (<>
                <button onClick={()=>pushRoute('/auth/login')} className="text-sm py-1 px-2 lg:text-base  lg:py-2 rounded-md md:px-5 border border-blue-500 font-semibold text-purple-500  bg-white ">Log In</button>
                <button onClick={()=>pushRoute('/auth/signup')} className="text-sm py-1 px-2 lg:text-base  lg:py-2 rounded-md md:px-5 border bg-purple-500 font-semibold text-white" >Sign Up</button>
                </>) :
                (<>
                <Link href='/user'>
